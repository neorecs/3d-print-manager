  ...
  return (
    <div className="space-y-4">
      <div className="grid gap-3 md:grid-cols-[minmax(180px,260px)_160px_160px_auto] md:items-end">
        ...
        <label className="space-y-2">
          <span className="text-sm font-bold text-slate-700">Max orders</span>
          <input className="w-full rounded-md border border-line bg-white px-3 py-2 text-sm outline-none focus:border-brand" max="250" min="1" onChange={(event) => setLimit(event.target.value)} type="number" value={limit} />
        </label>
        <label className="space-y-2">
          <span className="text-sm font-bold text-slate-700">Paginaformaat</span>
          <input className="w-full rounded-md border border-line bg-white px-3 py-2 text-sm outline-none focus:border-brand" max="50" min="1" onChange={(event) => setPageSize(event.target.value)} type="number" value={pageSize} />
        </label>
        <div className="flex flex-wrap gap-2">
          <button className="rounded-md bg-brand px-4 py-2 text-sm font-bold text-white disabled:opacity-60" disabled={Boolean(busy)} onClick={() => importOrders("shopify")} type="button">
            {busy === "shopify" ? "Shopify importeren..." : "Shopify orders importeren"}
          </button>
          <button className="rounded-md border border-line bg-white px-4 py-2 text-sm font-bold text-slate-700 disabled:opacity-60" disabled={Boolean(busy)} onClick={() => importOrders("etsy")} type="button">
