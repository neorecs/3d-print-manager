...
export function AIProductAssistant({ status }: { status: AIProductStatus }) {
  ...
  return (
    <div className="space-y-5">
      ...
      <form className="rounded-lg border border-line bg-panelSoft p-4" onSubmit={generate}>
        <div className="mb-5 rounded-lg border border-line bg-slate-950/25 p-4">
        ...
        <div className="grid gap-4 md:grid-cols-2">
          <label className="space-y-2 md:col-span-2">
            <span className="text-sm font-bold text-slate-300">Productidee</span>
            <textarea
        ...
        <details className="mt-5 rounded-lg border border-line bg-slate-950/25 p-4">
          <summary className="cursor-pointer list-none">
            <div className="flex flex-wrap items-center justify-between gap-3">
              ...
              <span className="rounded-full bg-slate-950/35 px-3 py-1 text-xs font-bold text-slate-300">optioneel</span>
            </div>
          </summary>
        ...
        <div className="mt-4 flex justify-end">
          <button className="rounded-md bg-brand px-4 py-2 text-sm font-black text-slate-950 disabled:opacity-60" disabled={busy || !input.idea.trim()} type="submit">
      ...
      {result ? (
        <div className="space-y-4">
          <div className="rounded-lg border border-line bg-panelSoft p-4">
              ...
              {autoTranslate ? (
                <div className="mt-3 flex flex-wrap gap-2">
                  {TRANSLATION_OPTIONS.map((option) => (
                    <label
                      className={`rounded-md border px-3 py-2 text-sm font-bold ${translationLanguages.includes(option.code) ? "border-brand bg-brand/10 text-brand" : "border-line bg-slate-950/35 text-slate-300"}`}
                      key={option.code}
                    >
                      <input
            ...
            {saveResult ? (
              <div className="mt-4 rounded-md border border-emerald-400/25 bg-emerald-400/10 px-3 py-3 text-sm text-emerald-300">
                <div className="font-bold">Concept opgeslagen{savedProductId ? ` als product #${savedProductId}` : ""}.</div>
                {savedProductId ? (
                  <a className="mt-2 inline-block rounded-md border border-emerald-400/25 bg-slate-950/35 px-3 py-2 text-sm text-ink font-bold text-emerald-300" href={`/catalogus/${savedProductId}`}>
                    Product openen
                  </a>
                ) : null}
                {warnings.length ? (
